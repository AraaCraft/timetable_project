...
class PlanningService:
        ...
        for existant in existants:
            ...
            if (
                ...
                if existant.id_salle == nouveau.id_salle:
                    raise ValueError(
                        f"La salle '{nouveau.nom_salle}' est déjà occupée sur ce créneau."
                    )
                if existant.id_promotion == nouveau.id_promotion:
                    raise ValueError(
                        f"La promotion {nouveau.id_promotion} a déjà un cours sur ce créneau."
                    )
    ...
    def annuler_creneau(self, id_creneau: int) -> Creneau:
        with Session(engine) as session:
            ...
            if not cours:
                raise ValueError(
                    f"Impossible d'annuler : le cours avec l'ID {id_creneau} n'existe pas."
                )
            if cours.est_annule:
